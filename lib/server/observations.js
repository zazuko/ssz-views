...
async function csvResponse (res, next) {
  const { query, endpoint, resultDimensions } = res.locals
  ...
  return rawCsv.body.pipe(linestream()).pipe(replaceHeader(resultDimensions)).pipe(res)
}
  ...
  return express.Router()
    .use(async (req, res, next) => {
      try {
        ...
        res.locals.resultDimensions = dimensions.array.filter(dimension => dimension.isResult)
      } catch (err) {
        return next(err)
      }
