...
class ViewBuilder {
    ...
    for (const request of config.Abfrage) {
      for (const [, data] of Object.entries(request.Cubes)) {
        ...
        for (const property of properties) {
          if (property.unfold || Object.keys(property.filters).length !== 0) {
  ...
  async valuesOfProperty (property) {
    ...
    // TODO: workaround for wrong eq filter
    if (property.filters.eq || property.filters.Dimensionscode) {
      if (values.length !== 0) {
        values = values.filter(value => value.value.includes(property.filters.eq || property.filters.Dimensionscode))
      } else {
        values = [ns.code(property.filters.eq || property.filters.Dimensionscode)]
      }
    }
  ...
  async addFilter (view, dimension, property, config) {
    ...
    for (const [key, value] of Object.entries(config)) {
      ...
      switch (key) {
        case 'eq':
          view.addFilter(dimension.filter.eq(filterTerm(property, value, datatype), { func }))
          break

        case 'gt':
          view.addFilter(dimension.filter.gt(filterTerm(property, value, datatype), { func }))
          break

        case 'gte':
          view.addFilter(dimension.filter.gte(filterTerm(property, value, datatype), { func }))
          break

        case 'lt':
          view.addFilter(dimension.filter.lt(filterTerm(property, value, datatype), { func }))
          break

        case 'lte':
          view.addFilter(dimension.filter.lte(filterTerm(property, value, datatype), { func }))
          break
